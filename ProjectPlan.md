...
Creazione del modello:  tramite importazione di file che contengono i dati sui vertici del modello.  
Texturing e Materiali: applicazione di texture e materiali per aggiungere realismo al modello.
Illuminazione : configurazione dell’illuminazione come aggiunta di fonti di luce.
Rendering: rendering finale per ottenere immagini di alta qualità.
...
progettazione iniziale: in questa fase studieremo i vari requisiti del progetto e definiremo obiettivi e funzionalità che il prodotto finale dovrà offrire all’utente. Verranno inoltre definiti i vari diagrammi UML necessari per la fase di implementazione;
Implementazione: partendo dai diagrammi definiti nella fase precedente, secondo un modello AGILE (scrum), per ogni sprint sarà definito uno sprint backlog con le varie attività da fare in quello sprint. Attraverso l’uso di una kanban board implementeremo poi le varie funzionalità;
Testing: il testing verrà fatto alla fine dell’implementazione di ogni gruppo di funzionalità correlate;
Revisione e Ottimizzazione: in questa fase effettueremo una revisione del modello e miglioreremo le prestazioni e la qualità visiva.
...
Eventuali utils o file loader: include funzioni di utilità generali e strumenti per il caricamento di file, come modelli 3D e texture.
